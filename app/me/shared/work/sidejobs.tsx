...
import HorizontalCardCarousel from './side-jobs/horizontal-card-carousel'
import Section from '../section'
import MobileSideJobs from './side-jobs/mobile-side-jobs'

type Job = {
  companyDescription: string
  companyName: string
  companyPostition: string
  companyUrl: string
  imageUrl: string
...
function SideJobs({mobile}: {mobile: boolean}) {
  const jobs = [
    {
  ...
  return (
    <Section>
        <div className='mb-16'>
          {
            mobile ?
            <MobileSideJobs jobs={jobs} />
            :
        <HorizontalCardCarousel jobs={jobs} />
          }
        </div>
    </Section>
  )
